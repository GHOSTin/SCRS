...
Meteor.methods({
    ...
    addResultsToJournal(data) {
        ...
        _.each(data.points, (elem, index, list)=>{
            ...
            if(_.isEmpty(elem[0])) {
                if(Meteor.isClient) {
                    console.warn(`Отклонено. Введены некорректные данные.`);
                }
            } else {
                ...
                if (exists) {
                    if(Meteor.isClient) {
                        console.log(exists);
                        console.warn(`Отклонено. Данные за период ${(new Date(data.startDate))} - ${(new Date(data.endDate))} уже существуют`);
                    }
                } else {
                    Journal.insert({
                        studentId: index,
