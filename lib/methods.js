...
Meteor.methods({
    ...
    addProfToStudent: (data) => {
        ...
        let lastProfession = Profession2Student.findOne({studentId: data.id}, {sort: {createAt: -1}});
        if(lastProfession){
            let monthCounts = Journal.find({p2s: lastProfession._id}).count();
            console.log(monthCounts);
            if (monthCounts < 12) {
                if(Meteor.isClient) {
                    Bert.alert(`Отклонено. Предыдущая профессия в процессе получения.`, 'danger', 'fixed-top', 'fa-warning');
                }
                throw {
                    message: 'Отклонено. У студента не закрыта предыдущая практика.',
                    code: 403
                }
            }
        } else {
            if(Meteor.isClient) {
                Bert.alert(`Профессия: успешно назначена студенту.`, 'success');
            }
        }
    }
});