...
Meteor.methods({
  addUser: function (userAttributes) {
    ...
    editStudent( id, data){
        check( Meteor.userId(), String );
        check(id, String);
        Students.update(id, {$set: {name: data.name, speciality: data.speciality, year: data.year}});
        if(data.profName){
            Profession2Student.update({studentId: id, isClosed: false}, {$set:
                {
                    profId: data.profName,
                    gild: data.gild,
                    sector: data.sector,
        ...
        if(Meteor.isClient){
            Bert.alert(`Редактирование прошло успешно.`, 'success');
        }
        return true;
    },
    deleteStudent: ( id )=>{
    ...
    let lastProfession = Profession2Student.findOne({
  ...
  addResultsToJournal(data) {
    ...
    _.each(data.points, (elem, index, list) => {
      ...
      let exists = Journal.findOne({
        studentId: index,
        profId: lastProfession.profId,
        startDate: data.startDate,
        endDate: data.endDate
      });
      let count = Journal.find({
        studentId: index,
        profId: lastProfession.profId,
      }).count();
      if (exists) {
        if (Meteor.isClient) {
          let student = Students.find({_id: index});
          throw new Meteor.Error('danger', `Отклонено. Данные за период ${(new Date(data.startDate))} - ${(new Date(data.endDate))} студента ${student.name} уже существуют`);
        }
      } else if (count === 12) {
        throw new Meteor.Error('danger', `Отклонено. Достигнуто максимальное количество отчетных недель.`);
      } else {
        Journal.insert({
          studentId: index,
          profId: lastProfession.profId,
